...
def get_mocked_bot() -> Bot:
    ...
    bot.fetch_user = AsyncMock(Bot.fetch_user,
                               side_effect=lambda user_id: user if str(user_id) == test_user_id else user2)
...
def setup_bot_and_cog(setup_func: Callable[[Bot], None]) -> discord.ext.commands.Cog:
    p_vars.bot = get_mocked_bot()
    setup_func(p_vars.bot)
    print(p_vars.bot.cogs)
    return camp_cog.CampaignCog()
