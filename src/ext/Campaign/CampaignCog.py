...
class CampaignCog(Cog):
    ...
    async def edit_char(self, ctx: ApplicationContext, char_tag: str = None):
        return await commands.edit_char_view(await init_context(ctx=ctx), char_tag)
    ...
    async def add_c(self, ctx: ApplicationContext, char_tag: str, char_name: str, user_id: str = None):
        return await commands.add_c(await init_context(ctx=ctx), char_tag, char_name, user_id)
    ...
    async def rem_c(self, ctx: ApplicationContext, char_tag: str):
        return await commands.rem_c(await init_context(ctx=ctx), char_tag)
    ...
    async def retag_pc(self, ctx: ApplicationContext, char_tag_old: str, char_tag_new: str):
        return await commands.retag_pc((await init_context(ctx=ctx)), char_tag_old, char_tag_new)
    ...
    async def rename_pc(self, ctx: ApplicationContext, char_tag: str, char_name_new: str):
        return await commands.rename_pc((await init_context(ctx=ctx)), char_tag, char_name_new)

    @slash_command(name="session", description="increase session counter by 1")
    async def session(self, ctx: ApplicationContext):
        return await commands.session((await init_context(ctx=ctx)))
    ...
    async def load_command(self, ctx: ApplicationContext, file_name: str):
        return await commands.load_command((await init_context(ctx=ctx)), file_name)
    ...
    async def claim(self, ctx: ApplicationContext, char_tag: str, user_id: str = None):
        return await commands.claim((await init_context(ctx=ctx)), char_tag, user_id)
    ...
    async def unclaim(self, ctx: ApplicationContext, character_tag: str = None):
        return await commands.unclaim((await init_context(ctx=ctx)), character_tag)
    ...
    async def add_player(self, ctx: ApplicationContext, user_id: str):
        return await commands.add_player((await init_context(ctx=ctx)), user_id)
    ...
    async def rem_player(self, ctx: ApplicationContext, user_id: str):
        return await commands.rem_player((await init_context(ctx=ctx)), user_id)
    ...
    async def cache(self, ctx: ApplicationContext):
        return await commands.cache((await init_context(ctx=ctx)))
    ...
    async def get_cache(self, ctx: ApplicationContext):
        return await commands.get_cache((await init_context(ctx=ctx)))
    ...
    async def undo(self, ctx: ApplicationContext, amount: int = 1):
        return await commands.undo(await init_context(ctx=ctx), amount)

    @slash_command(name="redo", description="Redo your undone non-mistakes")
    async def redo(self, ctx: ApplicationContext, amount: int = 1):
        return await commands.redo(await init_context(ctx=ctx), amount)
    ...
    async def log(self, ctx: ApplicationContext, adv=False):
        return await commands.log((await init_context(ctx=ctx)), adv)

    # ####################################################################################
    # these are kept around for unit testing purposes, but are not actual commands anymore
    # ####################################################################################
    ...
    async def crit(self, ctx: ApplicationContext, amount: int = 1, char_tag: str = None):
        return await commands.crit((await init_context(ctx=ctx)), amount, char_tag)
    ...
    async def faint(self, ctx: ApplicationContext, amount: int = 1, char_tag: str = None):
        return await commands.faint((await init_context(ctx=ctx)), amount, char_tag)
    ...
    async def dodged(self, ctx: ApplicationContext, amount: int = 1, char_tag: str = None):
        return await commands.dodged((await init_context(ctx=ctx)), amount, char_tag)
    ...
    async def cause(self, ctx: ApplicationContext, amount: int, kills: int = 0, char_tag: str = None):
        return await commands.cause((await init_context(ctx=ctx)), amount, kills, char_tag)
    ...
    async def take(self, ctx: ApplicationContext, amount: int, char_tag: str = None):
        return await commands.take((await init_context(ctx=ctx)), amount, char_tag)
    ...
    async def take_reduced(self, ctx: ApplicationContext, amount: int, char_tag: str = None):
        return await commands.take_reduced((await init_context(ctx=ctx)), amount, char_tag)
    ...
    async def heal(self, ctx: ApplicationContext, amount: int, char_tag: str = None):
        return await commands.heal((await init_context(ctx=ctx)), amount, char_tag)
