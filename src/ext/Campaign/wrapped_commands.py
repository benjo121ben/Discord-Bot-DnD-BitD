...
async def catch_and_respond_char_action(ctx: BridgeExtContext, char_tag: str, func: Callable[[str, str], str]) -> bool:
    """
...
async def catch_and_respond_file_action(ctx: BridgeExtContext, func: Callable[[str], str]) -> bool:
    """
...
async def catch_async_file_action(ctx: BridgeExtContext, func: Callable[[str], Awaitable[None]]) -> bool:
    """
        Wraps a file specific command function, executing it with the user_id gained from the context.
...
async def add_c(ctx: BridgeExtContext, char_tag: str, char_name: str, user_id: str = None) -> bool:
...
async def rem_c(ctx: BridgeExtContext, char_tag: str) -> bool:
    return await catch_and_respond_file_action(ctx,
                                               lambda executing_user: bcom.rem_char(executing_user, char_tag))


async def crit(ctx: BridgeExtContext, amount: int = 1, char_tag: str = None) -> bool:
    return await catch_and_respond_char_action(ctx,
                                               char_tag,
                                               lambda executing_user, tag: bcom.crit(executing_user, tag, amount))


async def faint(ctx: BridgeExtContext, amount: int = 1, char_tag: str = None) -> bool:
    return await catch_and_respond_char_action(ctx,
                                               char_tag,
                                               lambda executing_user, tag: bcom.faint(executing_user, tag, amount))


async def dodged(ctx: BridgeExtContext, amount: int = 1, char_tag: str = None) -> bool:
    return await catch_and_respond_char_action(ctx,
                                               char_tag,
                                               lambda executing_user, tag: bcom.dodge(executing_user, tag, amount))


async def cause(ctx: BridgeExtContext, amount: int, kills: int = 0, char_tag: str = None) -> bool:
    return await catch_and_respond_char_action(ctx,
                                               char_tag,
...
async def take(ctx: BridgeExtContext, amount: int, char_tag: str = None) -> bool:
    return await catch_and_respond_char_action(ctx,
                                               char_tag,
...
async def take_reduced(ctx: BridgeExtContext, amount: int, char_tag: str = None) -> bool:
    return await catch_and_respond_char_action(ctx,
                                               char_tag,
                                               lambda executing_user, tag: bcom.take_damage(executing_user, tag, amount,
                                                                                            True))
...
async def heal(ctx: BridgeExtContext, amount: int, char_tag: str = None) -> bool:
    return await catch_and_respond_char_action(ctx,
                                               char_tag,
                                               lambda executing_user, tag: bcom.heal(executing_user, tag, amount))


async def log(ctx: BridgeExtContext, adv=False) -> bool:
    return await catch_and_respond_file_action(ctx,
                                               lambda executing_user: bcom.log(executing_user, adv))


async def retag_pc(ctx: BridgeExtContext, char_tag_old: str, char_tag_new: str) -> bool:
    return await catch_and_respond_file_action(ctx,
                                               lambda executing_user: bcom.retag_character(executing_user, char_tag_old,
                                                                                           char_tag_new))


async def rename_pc(ctx: BridgeExtContext, char_tag: str, new_char_name: str) -> bool:
    return await catch_and_respond_file_action(ctx,
                                               lambda executing_user: bcom.rename_character(executing_user, char_tag,
                                                                                            new_char_name))


async def load_command(ctx: BridgeExtContext, file_name: str) -> bool:
    return await catch_and_respond_file_action(ctx,
                                               lambda executing_user: bcom.load_or_create_save(executing_user,
                                                                                               file_name))


async def add_player(ctx: BridgeExtContext, user_id: str) -> bool:
    return await catch_and_respond_file_action(ctx,
                                               lambda executing_user: bcom.add_player(executing_user, user_id))


async def rem_player(ctx: BridgeExtContext, user_id: str) -> bool:
    return await catch_and_respond_file_action(ctx,
                                               lambda executing_user: bcom.remove_player(executing_user, user_id))


async def claim(ctx: BridgeExtContext, char_tag: str, user_id: str = None) -> bool:
    return await catch_async_file_action(ctx,
                                         lambda executing_user: bcom.claim_character(executing_user, ctx, char_tag,
                                                                                     user_id))


async def unclaim(ctx: BridgeExtContext, character_tag: str = None) -> bool:
    return await catch_and_respond_char_action(ctx,
                                               character_tag,
                                               lambda executing_user, tag: bcom.unclaim_char(executing_user, tag))


async def session(ctx: BridgeExtContext) -> bool:
    executing_user = str(ctx.author.id)
    try:
...
async def cache(ctx: BridgeExtContext) -> bool:
    return await catch_async_file_action(ctx,
                                         lambda executing_user: bcom.cache_file(executing_user, ctx))


async def get_cache(ctx: BridgeExtContext) -> bool:
    try:
...
async def download(ctx: BridgeExtContext) -> bool:
    executing_user = str(ctx.author.id)
    try:
...
async def undo(ctx: BridgeExtContext, amount: int = 1) -> bool:
    return await catch_and_respond_file_action(ctx,
                                               lambda executing_user: bcom.undo_command(executing_user, amount))


async def redo(ctx: BridgeExtContext, amount: int = 1) -> bool:
    return await catch_and_respond_file_action(ctx,
